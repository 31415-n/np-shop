...
-- Реализация HTTP-запроса через компонент internet
...
-- Реализация записи в файл через OpenOS filesystem
...
-- Заменить tmpfs на filesystem в функции time
local function time(raw)
  local handle = filesystem.open("/tmp/time", "w")
  handle:write("time")
  handle:close()
